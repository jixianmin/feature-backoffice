...
const UpgradePlan = () => {
    ...
    return (<div className='modal fade' id='kt_modal_upgrade_plan' aria-hidden='true'>
          ...
          <div className='modal-body pt-0 pb-15 px-5 px-xl-20'>
            ...
            <div className='d-flex flex-column'>
              <div className='nav-group nav-group-outline mx-auto' data-kt-buttons='true'>
                <a href='#' className={'btn btn-color-gray-400 btn-active btn-active-secondary px-6 py-3 me-2 ' +
            (currentState === 'month' && 'active')} onClick={() => {
            setCurrentState('month');
        }} data-kt-plan='month'>
                  Monthly
                </a>
                <a href='#' className={'btn btn-color-gray-400 btn-active btn-active-secondary px-6 py-3 me-2 ' +
            (currentState === 'annual' && 'active')} onClick={() => {
            ...
            return (<div onClick={() => {
                    ...
                    (plan.default && 'active ') +
                    (!plan.custom && 'btn-active btn-active-primary ')} data-bs-toggle='tab' data-bs-target={`#kt_upgrade_plan_${index}`} key={index}>
                          <div className='d-flex align-items-center me-2'>
                            <div className='form-check form-check-custom form-check-solid form-check-success me-6'>
                              <input className='form-check-input' type='radio' name='plan' value={plan.title} checked={selected === plan.title} onChange={(e) => setSelected(e.target.value)}/>
                            </div>
                            ...
                            <div className='flex-grow-1'>
                              <h2 className='d-flex align-items-center fs-2 fw-bolder flex-wrap'>
                                ...
                                {plan.label && (<span className='badge badge-light-success ms-2 fs-7'>
                                    {plan.label}
                                  </span>)}
                              </h2>
                          ...
                          <div className='ms-5'>
                            {plan.custom && (<button className='btn btn-sm btn-primary'>Contact Us</button>)}
                            {!plan.custom && (<>
                ...
                <div className='col-lg-6'>
                  <div className='tab-content rounded h-100 bg-light p-10'>
                    {plans.map((plan, index) => {
            return (<div key={`custom${index}`}>
                          {!plan.custom && (<>
                              <div className={`tab-pane fade` + (plan.default && 'show active')} id={`kt_upgrade_plan_${index}`} key={index}>
                                <div className='pb-5'>
                        ...
                        return (<div className={`d-flex align-items-center` +
                                (i !== plan.features.length - 1 && ' mb-7')} key={`${i}-${feature.title}`}>
                                        {feature.supported && (<>
